...
export function MatchCard({ fixture }: { fixture: FixtureWithAll }) {
  ...
  return (
    <>
      <Card className="group p-4 hover:border-cyan-500/40 transition-all duration-200 cursor-pointer" onClick={() => setOpen(true)}>
        {/* League row */}
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            {fixture.league.logo && (
              <Image src={fixture.league.logo} alt={fixture.league.name} width={16} height={16} className="rounded-sm" unoptimized />
            )}
        ...
        <div className="flex items-center justify-between gap-3 mb-4">
          <TeamBlock name={fixture.homeTeam.name} logo={fixture.homeTeam.logo} align="left" />
          ...
          <TeamBlock name={fixture.awayTeam.name} logo={fixture.awayTeam.logo} align="right" />
        </div>
        ...
        {a ? (
          <>
            {/* Probability bar */}
            <ProbabilityBar home={a.homeWinPct} draw={a.drawPct} away={a.awayWinPct} />

            <div className="mt-3 flex items-center flex-wrap gap-1.5">
              <Badge variant="teal">🎯 {favoriteLabel}</Badge>
              <Badge variant={confidenceVariant}>
                Confiança: {a.confidence === "high" ? "Alta" : a.confidence === "medium" ? "Média" : "Baixa"}
              </Badge>
              <Badge variant={drawVariant}>
                Empate: {a.drawTendency === "high" ? "Alta" : a.drawTendency === "medium" ? "Média" : "Baixa"}
              </Badge>
            </div>
          </>
        ) : (
        ...
        {/* CTA */}
        <button className="mt-3 w-full text-center text-xs text-cyan-400 hover:text-cyan-300 transition-colors border border-cyan-500/20 rounded-lg py-1.5 hover:bg-cyan-500/10">
          Ver análise completa →
        </button>
      </Card>

      {open && (
        <AnalysisModal fixture={fixture} onClose={() => setOpen(false)} />
      )}
    </>
  );
}
...
function TeamBlock({ name, logo, align }: { name: string; logo: string | null; align: "left" | "right" }) {
  return (
    <div className={`flex flex-col items-${align === "left" ? "start" : "end"} gap-1.5 flex-1`}>
      {logo && (
        <Image src={logo} alt={name} width={32} height={32} unoptimized className="rounded-sm" />
      )}
